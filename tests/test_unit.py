...
from tf_types import (
    RunConfig, Mode, Strictness, RunStatus, EvalDecision,
    RetrievedChunk, StepRecord, TraceRecord
)
...
class TestEvaluatorNode:
    ...
    def test_evaluator_detects_risk_keywords(self):
        ...
        assert "rm -rf" in step["input_data"]["risk_flags"] or "drop" in step["input_data"]["risk_flags"]
...
class TestRouterNode:
    ...
    def test_router_returns_fallback_message(self):
        ...
        assert "sorry" in result["final_answer"].lower() or "couldn't" in result["final_answer"].lower()
...
class TestTraceStore:
    ...
    def test_create_trace_inserts_record(self, mock_connection):
        ...
        with patch('persistence.trace_store.Sqlite') as mock_sqlite:
            ...
            insert_calls = [c for c in mock_connection.execute.call_args_list 
                          if "INSERT INTO traces" in str(c)]
