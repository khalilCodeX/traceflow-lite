...
def route_after_evaluator(state: TraceFlowState):
    ...
    if state.eval_report and state.eval_report.decision == EvalDecision.PASS:
        return "end"
    if state.eval_report and state.eval_report.decision == EvalDecision.REVISE and state.revisions < state.config.max_revisions:
        return "executor"
    return "router"
...
def build_traceflow_graph():
    ...
    graph.add_conditional_edges("planner", router_after_planner, {
        "retriever": "retriever",
        "executor": "executor"
    })
