...
def _get_revision_instructions(mode: Mode, citation_ok: bool, schema_ok: bool, risks: list[str]) -> str:
...
def evaluator_node(state: TraceFlowState) -> dict:
    ...
    if draft:
        if config.mode == Mode.GROUNDED_QA:
            ...
            mentions_source = any(
                chunk.source.lower() in draft.content.lower()
            ...
            risk_keywords = ['delete', 'drop', 'truncate', 'remove all', 
                             'rm -rf', 'destroy', 'production', 'prod', 
                             'force', '--force', 'sudo']
    ...
    if cost_exceeded or latency_exceeded:
        if revision_limit_reached:
            report = EvalReport(
                ...
                reasons=[f"Cost: {draft.cost_usd:.4f}, Latency: {draft.latency_ms:.2f}ms exceeded limits"],
...
def router_node(state: TraceFlowState) -> dict:
    ...
    steps.append({
        "node_name": "router",
        "input_data": {"eval_decision": state.eval_report.decision.value if state.eval_report else None},
