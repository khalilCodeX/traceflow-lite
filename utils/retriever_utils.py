...
class chroma_retriever:
    ...
    def create_vector_store(self, splits: list[str], batch_size: int = 1000):
        for batch in tqdm(self.create_document_batches(splits, batch_size), desc="embedding batches", unit="batch"):
    ...
    def retrieve_similar_docs(self, query: str, n_results: int = 5) -> list[RetrievedChunk]:
        ...
        for doc, metadata, distance in zip(results["documents"][0], results["metadatas"][0], results["distances"][0]):
            retrieved_chunks.append(RetrievedChunk(chunk_id=metadata.get("source", ""), content=doc, metadata=metadata, relevance_score=(1.0 - distance/2), source=metadata.get("source", "")))
